	...
	onMount(async () => {
		for (const [key, value] of Object.entries(liquidations.XBTUSD.data)) {
			...
			value.map(item => {
				...
				timestamp =
					new Date(
						Number.parseFloat(item.timestamp._seconds + '000').toPrecision()
					).getHours() + ':00';
			});
			const hourlyObject = {
				timestamp: timestamp,
...
<Card>
	<h4 class="symbol">XBTUSD <span class="price">{Number.parseFloat(totalBuy + totalSell).toPrecision().toLocaleString()}</span></h4>
	<canvas id="xbtChart" style="width:100%" />
	<div class="graph-legend">
		<div>{Number.parseFloat(totalBuy).toPrecision().toLocaleString()}</div>
		...
		<div>{Number.parseFloat(totalSell).toPrecision().toLocaleString()}</div>
	</div>
</Card>
